...
#include "xdppsu_hw.h"
#include "xstatus.h"
/* #include <math.h>  -- removed; nearbyint replaced with integer math */
...
void XDpPsu_CfgMsaUseEdidPreferredTiming(XDpPsu *InstancePtr, u8 *Edid)
{
	...
	/* Patched: integer division to avoid pulling in nearbyint() / FP. The
	 * +half-divisor term mimics nearbyint() rounding to nearest. -- bard0
	 * design 2026, for the bare-metal KV260 DP project. */
	{
		u32 _denom = (u32)(MsaConfig->Vtm.Timing.HTotal *
				   MsaConfig->Vtm.Timing.F0PVTotal);
		MsaConfig->Vtm.FrameRate = (_denom == 0u) ? 0u :
			(u32)((MsaConfig->PixelClockHz + (_denom / 2u)) / _denom);
	}
