...
- An MMCM-derived clock domain runs everything: 50 MHz in the default
...
The default build (4 pixels per clock) runs at **50 MHz** -- 200 Mpixel/s --
with WNS = +5.252 ns (timing met, 0 failing endpoints) on the internal
...
`DIVCLK_DIVIDE=2`) and divides it by `CLKOUT0_DIVIDE_F`: 13 for PPC>1
(50 MHz), 5 for PPC=1 (130 MHz). The per-lane checker/grid counter chains do
not close 130 MHz at PPC>1, and the demo targets correctness rather than
throughput. `build.tcl` prints the clock each build was constrained at.
...
│   ├── clk_gen.v                MMCM 100 → 130 / 50 MHz + reset sync
