...
module vtpgz_core #(
    ...
    function [35:0] bar_palette;
        ...
        begin
            if (OUTPUT_MODE == `VTPGZ_MODE_YUV) begin
                // Four constant palettes, {BT.601, BT.709} x {full, limited},
                // 100% bars as 12-bit {Y, Cb, Cr}. YUV_RANGE and YUV_MATRIX
                // are elaboration constants, so exactly one arm survives and
                // the other three cost nothing.
                //
                // Generated by scripts/gen_yuv_palettes.py from the
                // colorimetry, NOT transcribed -- the Python model derives
                // its tables from the same script, so neither side is a copy
                // of the other and a wrong constant cannot agree with itself.
                //
                // The limited constants are the standard 10-bit levels x 4,
                // so the pack stage's truncation lands on the exact standard
                // code at every BPC: 3760>>2 = 940, 3760>>4 = 235, 256>>4 = 16.
                //
                // BT.601 full is the shipped default and is FROZEN: its
                // green and magenta chroma sit one LSB from what the formula
                // gives (the original used a different rounding), and that
                // LSB survives to the output at BPC>=10. Regenerating it
                // would change every existing 10- and 12-bit build, so it is
                // kept verbatim and the generator asserts the difference
                // stays within one LSB.
                if (YUV_MATRIX == `VTPGZ_YUV_BT709) begin
                    if (YUV_RANGE == `VTPGZ_YUV_LIMITED) begin
                        case (idx)
                            3'd0: bar_palette = {12'hEB0, 12'h800, 12'h800}; // white
                            3'd1: bar_palette = {12'hDB4, 12'h100, 12'h8A4}; // yellow
                            3'd2: bar_palette = {12'hBC8, 12'h99C, 12'h100}; // cyan
                            3'd3: bar_palette = {12'hACC, 12'h29C, 12'h1A4}; // green
                            3'd4: bar_palette = {12'h4E4, 12'hD64, 12'hE5C}; // magenta
                            3'd5: bar_palette = {12'h3E8, 12'h664, 12'hF00}; // red
                            3'd6: bar_palette = {12'h1FC, 12'hF00, 12'h75C}; // blue
                            default: bar_palette = {12'h100, 12'h800, 12'h800}; // black
                        endcase
                    end else begin
                        case (idx)
                            3'd0: bar_palette = {12'hFFF, 12'h800, 12'h800}; // white
                            3'd1: bar_palette = {12'hED7, 12'h000, 12'h8BC}; // yellow
                            3'd2: bar_palette = {12'hC98, 12'h9D5, 12'h000}; // cyan
                            3'd3: bar_palette = {12'hB71, 12'h1D6, 12'h0BC}; // green
                            3'd4: bar_palette = {12'h48E, 12'hE2A, 12'hF44}; // magenta
                            3'd5: bar_palette = {12'h367, 12'h62B, 12'hFFF}; // red
                            3'd6: bar_palette = {12'h128, 12'hFFF, 12'h744}; // blue
                    ...
                    if (YUV_RANGE == `VTPGZ_YUV_LIMITED) begin
                        case (idx)
                            3'd0: bar_palette = {12'hEB0, 12'h800, 12'h800}; // white
                            3'd1: bar_palette = {12'hD20, 12'h100, 12'h924}; // yellow
                            3'd2: bar_palette = {12'hA98, 12'hA5C, 12'h100}; // cyan
                            3'd3: bar_palette = {12'h908, 12'h35C, 12'h224}; // green
                            3'd4: bar_palette = {12'h6A8, 12'hCA4, 12'hDDC}; // magenta
                            3'd5: bar_palette = {12'h518, 12'h5A4, 12'hF00}; // red
                            3'd6: bar_palette = {12'h290, 12'hF00, 12'h6DC}; // blue
                            default: bar_palette = {12'h100, 12'h800, 12'h800}; // black
                        endcase
                    end else begin
                        // FROZEN default -- see the note above.
                        case (idx)
                            3'd0: bar_palette = {12'hFFF, 12'h800, 12'h800}; // white
    ...
    //   * IMAGE -- carries RGB, not luma (see the known-issue note in README);
